...
def get_hist_score(tr_probs, te_probs, bins=10):
  tr_probs_np = tr_probs.detach().numpy()
  te_probs_np = te_probs.detach().numpy()

  # Create histogram for training probabilities
  tr_heights, bin_edges = np.histogram(tr_probs_np, bins=bins, density=True)

  # Create histogram for test probabilities with the same bin edges
  te_heights, _ = np.histogram(te_probs_np, bins=bin_edges, density=True)

  score = 0
  # Loop through bins to compute the histogram intersection score
  for i in range(len(bin_edges) - 1):
      bin_diff = bin_edges[i + 1] - bin_edges[i]

      # Compute area under the histogram within the bin for training and test probabilities
      tr_area = bin_diff * tr_heights[i]
      te_area = bin_diff * te_heights[i]

      # Compute the intersection and add to the score
...
class MonitoringSystem:
  ...
  def calibrate(self, tr_probs, tr_labels, te_probs):
    ...
    tr_probs_cal = iso_reg.fit_transform(tr_probs.numpy(), tr_labels.numpy())
    ...
    return torch.from_numpy(tr_probs_cal), torch.from_numpy(te_probs_cal)
